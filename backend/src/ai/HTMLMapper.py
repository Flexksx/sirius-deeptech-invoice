...
def generate_html(invoice_get_response):
    invoice_data = invoice_get_response['invoice_type']
    ...
    html_output = html_template.format(
        ...
        obligor_idno=f'<p>IDNO: {
            obligor_data["idno"]}</p>' if obligor_data['idno'] else '',
        ...
        obligor_email=f'<p>Email: {
            obligor_data["email"]}</p>' if obligor_data.get('email') else '',
        obligor_fiscal_code=f'<p>Fiscal Code: {
            obligor_data["fiscal_code"]}</p>' if obligor_data.get('fiscal_code') else '',
        obligor_iban=f'<p>IBAN: {
            obligor_data["iban"]}</p>' if obligor_data.get('iban') else '',
        obligor_phone=f'<p>Phone: {
            obligor_data["phone"]}</p>' if obligor_data.get('phone') else '',
        obligor_tva_code=f'<p>TVA Code: {
            obligor_data["tva_code"]}</p>' if obligor_data.get('tva_code') else '',
        ...
        obligee_email=f'<p>Email: {
            obligee_data["email"]}</p>' if obligee_data.get('email') else '',
        obligee_fiscal_code=f'<p>Fiscal Code: {
            obligee_data["fiscal_code"]}</p>' if obligee_data.get('fiscal_code') else '',
        obligee_iban=f'<p>IBAN: {
            obligee_data["iban"]}</p>' if obligee_data.get('iban') else '',
        obligee_idno=f'<p>IDNO: {
            obligee_data["idno"]}</p>' if obligee_data.get('idno') else '',
        obligee_phone=f'<p>Phone: {
            obligee_data["phone"]}</p>' if obligee_data.get('phone') else '',
        obligee_tva_code=f'<p>TVA Code: {
            obligee_data["tva_code"]}</p>' if obligee_data.get('tva_code') else '',
...
# file_path = os.path.join(os.path.dirname(__file__), 'html_template/a.json')
...
#     output_file.write(generate_html(data))
