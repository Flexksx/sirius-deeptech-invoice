...
<Modal title="Upload Contract" classHeader="bg-lightBlue" classBody="border-lightGray" classFooter="bg-lightBlue" classBackdrop="bg-lightGray" class="bg-darkBlue border-lightGray" size="lg" bind:open={defaultModal}>
    {#if (updated === false && loading === false)}
        <div transition:slide class="mt-5 p-5 rounded-lg flex flex-col justify-center items-center">
            <Dropzone
    ...
    {#if loading === false && updated === false}
        <div class="flex justify-end space-x-2">
            <Button outline color="red" on:click={() => { defaultModal = false; value = []; }}>Cancel</Button>
            <Button outline on:click={() => uploadFiles(value)}>Confirm</Button>
        </div>
    {/if}

    {#if updated === true && loading === false}
